  ...
  const fetchServices = async (city) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/services/get?city=${city}`
      );
  ...
  const CustomDropdownToggle = React.forwardRef(
    ({ children, onClick }, ref) => {
      ...
      return (
        ...
        >
          <img src={profileImgPath} alt="Profile" className="profile-img" />
        </div>
      );
    }
  );
    ...
    <div className="home-page">
      <Container>
        ...
        <Row>
          <Col
            md={6}
            className={`content-box p-4 ${loggedIn ? "center-content" : ""}`}
          >
            <h2 className={`mb-4 head ${loggedIn ? "center-head" : ""}`}>
              GENERAL BIKE SERVICE IN INDIA
            </h2>
            <p className="content-p">
              Keep your bike in top gear with{" "}
              <span className="highlighted-text">Gear-Up Garage's</span>{" "}
              hassle-free pickup and drop service across India. Book online now
              and let our expert mechanics handle the rest, right from your
              doorstep!
            </p>
          </Col>
          <Col md={6} className="login-button-container">
            {!loggedIn && (
              <div className="login-text">Login to enjoy the service</div>
            )}
            {loggedIn ? (
              <Dropdown>
                <Dropdown.Toggle
                ...
                <Dropdown.Menu>
                  ...
                  <Dropdown.Item onClick={handleViewBooking}>
                    View Booking
                  </Dropdown.Item>
        ...
        <div className={`book-easy ${loggedIn ? "center-book" : ""}`}>
          <h className="htag">SIMPLE & easy to book</h>
          <div>
              ...
              <div className="step-container">
                ...
                <div className="step">
                  <div className="step-title">
                    Get the service done with Free pickup & Drop
                  </div>
                </div>
              </div>
            </Row>
            ...
            <Carousel>
              {services.map((service) => {
                ...
                const truncatedDescription =
                  endIndex !== -1
                    ? service.description.substring(0, endIndex)
                    : service.description;
