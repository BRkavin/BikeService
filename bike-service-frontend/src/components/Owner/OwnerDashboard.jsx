...
const OwnerDashboard = () => {
  ...
  const fetchServices = async () => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/services?ownerEmail=${ownerEmail}&city=${city}`
      );
  ...
  const fetchBookings = async (city) => {
    try {
      const response = await axios.get(
        `http://localhost:5000/api/bookings?city=${city}`
      );
  ...
  const handleViewMessages = async () => {
    try {
      ...
      const city = localStorage.getItem("ownerCity");
      const response = await axios.get(
        `http://localhost:5000/api/contact?owneremail=${ownerEmail}&city=${city}`
      );
  ...
  const filterBookings = () => {
    const filtered = bookings.filter((booking) => {
      ...
      return (
        ...
        (booking.serviceDate &&
          new Date(booking.serviceDate).toLocaleDateString().includes(term)) ||
          ...
          <Modal.Body>
            {selectedBooking && (
              <div className="booking-details">
                ...
                <p>
                  <strong>Service Name:</strong> {selectedBooking.serviceName}
                </p>
                <p>
                  <strong>Customer Name:</strong> {selectedBooking.userName}
                </p>
                <p>
                  <strong>Mobile Number:</strong> {selectedBooking.mobileNumber}
                </p>
                <p>
                  <strong>Bike Brand:</strong> {selectedBooking.bikeBrand}
                </p>
                <p>
                  <strong>Bike Model:</strong> {selectedBooking.bikeModel}
                </p>
                <p>
                ...
                <p>
                  ...
                  {selectedBooking.problemDetails}
                </p>
                <p>
